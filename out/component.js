...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "./index", "./html", "./config", "fs", "process"], factory);
    }
    ...
    index_1.Clean.forEach(($, Num) => {
        $('import')
            ...
            if (data == undefined) {
                console.log('   ●'.red +
                    ` The component ${req} does not exist: ${$.prototype.name}`);
                process_1.exit();
            }
...
${data.innerText}`;
                ...
                if (data.attribs.type == 'html') {
                    ...
                    const renderString = $.html(Import).trim().split('</import>')[0] +
                        '</import>';
                    $.prototype.name = $.prototype.name.split('.vide')[0];
                    let origin;
                    let path;
