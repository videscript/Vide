  ...
  $.forEach(($: any) => {
    if (Object.keys($('Vide').get()[0].attribs).includes('script')) {
        ...
        if (element.children[0] === undefined) {
          ...
          col = {
            ...
            fullText: Clean.filter(
              (r: any) => r(`component[name=${element.attribs.name}]`).html() != null
            )[0],
            length: element.children.length
          };
        }
      ...
      if (config.outDir === undefined) {
        if (fs.existsSync(`./${$.prototype.name.split('.vide')[0] || 'html'}.html`)) {
          fs.unlinkSync(`./${$.prototype.name.split('.vide')[0] || 'html'}.html`);
        }
      } else if (
        fs.existsSync(`${config.outDir}/${$.prototype.name.split('.vide')[0] || 'html'}.html`)
      ) {
        fs.unlinkSync(`${config.outDir}/${$.prototype.name.split('.vide')[0]}.html`);
      }
      if (Object.keys(videAttr).includes('router')) {
        ...
        $('Router').remove();
        const render: string = $('Vide component').remove().end().children().html().trim();
        const fullTemp = `
<!DOCTYPE html>
<html lang="en">
<head>
...
`;
        if (`${config.outDir}/` === 'undefined/') {
          fs.appendFileSync(`./${$.prototype.name.split('.vide')[0] || 'html'}.html`, fullTemp);
        } else {
          try {
            fs.appendFileSync(
              `${config.outDir}/${$.prototype.name.split('.vide')[0] || 'html'}.html`,
              fullTemp
            );
          } catch (err) {
